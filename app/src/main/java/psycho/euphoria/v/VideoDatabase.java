...
public class VideoDatabase extends SQLiteOpenHelper {
    ...
    public List<Video> queryVideos(String search, int sortBy, int videoType) {
        ...
        if (sortBy == 0) {
            if (search == null) {
                cursor = getReadableDatabase().rawQuery("select * from videos where video_type = ? ORDER by create_at DESC LIMIT 3000", new String[]{Integer.toString(videoType)});
            } else {
                // video_type = ? and  Integer.toString(videoType),
                cursor = getReadableDatabase().rawQuery("select * from videos where video_type = ? and title like ? ORDER by create_at DESC", new String[]{Integer.toString(videoType), "%" + search + "%"});
            }
        } else if (sortBy == 1) {
            if (search == null) {
                cursor = getReadableDatabase().rawQuery("select * from videos where video_type = ? ORDER by create_at LIMIT 3000", new String[]{Integer.toString(videoType)});
            } else {
                cursor = getReadableDatabase().rawQuery("select * from videos where video_type = ? and title like ? ORDER by create_at ", new String[]{Integer.toString(videoType), "%" + search + "%"});
            }
        } else if (sortBy == 2) {
            if (search == null) {
                cursor = getReadableDatabase().rawQuery("select * from videos where video_type = ? ORDER by create_at LIMIT 3000", new String[]{Integer.toString(videoType)});
            ...
            if (search == null) {
                cursor = getReadableDatabase().rawQuery("select * from videos where video_type = ?  ORDER by views DESC LIMIT 3000", new String[]{Integer.toString(videoType)});
            } else {
                cursor = getReadableDatabase().rawQuery("select * from videos where video_type = ? and title like ? ORDER by views DESC ", new String[]{Integer.toString(videoType), "%" + search + "%"});
            }
        }
